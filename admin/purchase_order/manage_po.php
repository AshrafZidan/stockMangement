...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4 class="card-title"><?php echo isset($id) ? "تفاصيل الطلبية - ".$po_code : 'اضافة طلبية جديدة' ?></h4>
    </div>
    <div class="card-body">
        <form action="" id="po-form">
            ...
            <div class="container-fluid">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="supplier_id" class="control-label text-info">المورد</label>
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tfoot>
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">الاجمالى الجزئي</th>
                            <th class="text-right py-1 px-2 sub-total">0</th>
                        </tr>
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">الخصم 
                        ...
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">الضرائب <input style="width:40px !important" name="tax_perc" class='' type="number" min="0" max="100" value="<?php echo isset($tax_perc) ? $tax_perc : 0 ?>">%
                        ...
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">الاجمالى
                        ...
                        <tr>
                            <th class="text-right py-1 px-2" colspan="5">المدفوع
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="remarks" class="text-info control-label">ملاحظات الفاتوره</label>
    ...
    <div class="card-footer py-1 text-center">
        <button class="btn btn-flat btn-primary" type="submit" form="po-form">حفظ</button>
        <a class="btn btn-flat btn-dark" href="<?php echo base_url.'/admin?page=purchase_order' ?>">الغاء</a>
    </div>
</div>
<table id="clone_list" class="d-none">
    <tr>
        ...
        <td class="py-1 px-2 text-center qty">
            ...
            <input type="hidden" name="old_qty[]">
...
<script>
    ...
    // اجعل list_item دائماً عبارة عن Promise
    var list_item = Promise.resolve();

    if(!!items[supplier_id]){
        $('#item_id').html('');
        list_item = new Promise(resolve => {
    ...
    list_item.then(function(){
        $('#item_id').select2({
            placeholder: items[supplier_id] ? "اختار منتج من هنا" : "لا يوجد منتجات",
            width: 'resolve',
        });
    });
});
        ...
        $('#add_to_list').click(function(){
            ...
            if(item == '' || qty == '' || unit == '' ){
                alert_toast('يجب ادخال كل البيانات .','warning');
                return false;
            }
        ...
        $('#po-form').submit(function(e){
			...
			$.ajax({
				...
				error:err=>{
					console.log(err)
					alert_toast("حدث خطا فالتسجيل",'error');
					end_loader();
				},
				success:function(resp){
					if(resp.status == 'success'){
						location.replace(_base_url_+"admin/?page=purchase_order");
