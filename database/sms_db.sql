...
CREATE TABLE `item_list` (
  ...
  `consumer_price` float NOT NULL DEFAULT 0,
...
CREATE TABLE `purchase_order_list` (
  ...
  `status` tinyint(4) NOT NULL DEFAULT 2 COMMENT '0 = pending, 1 = partially received, 2 = received',
  `date_created` datetime NOT NULL DEFAULT current_timestamp(),
  `date_updated` datetime NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB;

-- Dumping data for table `purchase_order_list`
INSERT INTO `purchase_order_list` (`id`, `po_code`, `supplier_id`, `amount`, `discount_perc`, `discount`, `tax_perc`, `tax`, `remarks`, `status`, `date_created`, `date_updated`) 
VALUES 
...
INSERT INTO `supplier_list` (`id`, `name`, `address`, `cpersonContact`, `contact`, `status`, `date_created`, `date_updated`) VALUES
